...
output Valid_W, output [31:0] PCW, 
...
.StallM(StallM)
    ...
    wire MemAccessM;
    wire d_mem_ready;
    assign MemReadM   = (ResultSrcM == 2'b01) & Valid_M;
    assign MemAccessM = (MemWriteM | MemReadM) & Valid_M;
    
    ahb_dmem_master k11 (
    .clk(clk),
    .rst(rst),
    .mem_access(MemAccessM),
    .mem_write(MemWriteM & Valid_M),
    .addr(ALUResultM),
    .wdata(WriteDataM),
    .funct3(funct_3M),
    .rdata(RD),
...
,.MemWriteW(MemWriteW),.WriteDataW(WriteDataW),.StallW(StallW)
);
...
.ResultW(ResultW)
 ...
 wire MemStall;
 assign MemStall = (MemAccessM & ~d_mem_ready ) | ~i_HREADY;
